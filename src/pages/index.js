...
import {initialCards, cards, editButton,
        ...
        authorPopUpSmbBtn, addPlacePopUpSelector, placePhoto,
        placeName, cardsTempalteSelector, imgPopUpSelector, apiSettings,
...
function createCard(item) {
    ...
    openRemovePopUp, likeButtonClick, user._userId);
    return card;
}
...
function likeButtonClick (evt) {
    if (!(evt.target.classList[1] === 'element__like-btn_active')) {
        api.addLike(evt.target.offsetParent.id)
        .then(
            evt.target.parentElement.lastElementChild.textContent = parseInt(evt.target.parentElement.lastElementChild.textContent) + 1,
            evt.target.classList.add('element__like-btn_active')
        );
    }
    else {
        api.removeLike(evt.target.offsetParent.id)
        .then(
            evt.target.parentElement.lastElementChild.textContent = parseInt(evt.target.parentElement.lastElementChild.textContent) - 1,
...
const avatarPopUp = new PopupWithForm(avatarPopUpSelector, function (evt, inputValues) {
    evt.preventDefault();
    const button = evt.target.querySelector('.popup__submit-btn');
    button.value = 'Сохранение...';
